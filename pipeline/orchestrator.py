...
from ..integrations.feishu_reporter import notify_review_result
...
class ReviewOrchestrator:
    ...
    def __init__(
        ...
        # Workers use a separate client with thinking disabled for speed
    ...
    def review(self, pr_input: str) -> ReviewResult:
        ...
        # Post-review: notify Feishu with full review text
        self._notify_feishu(review_result, review_text=result.final_response)
    ...
    def multi_agent_review(self, pr_input: str) -> ReviewResult:
        ...
        scheduler = TaskScheduler(max_concurrency=3, per_task_timeout_s=300.0)
        ...
        # Post-review: notify Feishu (best-effort, failures are silent)
        self._notify_feishu(
            final_result,
    ...
    def _notify_feishu(
        ...
        """Post-review: push results to Feishu (best-effort, never raise)."""
        try:
            r = notify_review_result(
                pr_label=result.pr,
            ...
            if not r.get("ok"):
                import sys
                print(f"\n[feishu] Notification failed: {r.get('error', 'unknown')}", file=sys.stderr)
            else:
                print(f"\n[feishu] Review summary sent to chat", file=sys.stderr)
        except Exception as e:
            import sys
            print(f"\n[feishu] Notification error: {e}", file=sys.stderr)
